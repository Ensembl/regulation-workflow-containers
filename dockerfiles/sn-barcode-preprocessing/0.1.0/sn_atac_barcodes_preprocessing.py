...
def barcodes_match(fastq_file, whitelist,sample_size,offset, thresh,use_pigz, cores):
    ...
    for title, bc, plus, qual in bc_sample:
        if bc.split()[0][offset:] in whitelist:
            bc_match += 1
        if bc.split()[0][offset:] in rc_whitelist:
            bcrc_match += 1
...
if __name__ == "__main__":
    ...
    # loading whitelist
    with open_file(whitelist_file,  use_pigz, num_cores,  "rt") as f:
        whitelist = [line.strip() for line in f if line.strip()]
        
    # determine if reverse and complement operation is needed
    rc_need = barcodes_match(fastq_file, set(whitelist), sample_size, offset, min_prop_dir,  use_pigz, num_cores )
    if rc_need is None:
        exit()
